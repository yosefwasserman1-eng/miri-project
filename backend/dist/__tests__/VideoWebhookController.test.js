...
(0, vitest_1.describe)('VideoWebhookController - POST /api/webhooks/video', () => {
    let app;
    (0, vitest_1.beforeAll)(async () => {
        app = (0, express_1.default)();
        app.use(express_1.default.json());
        const { VideoWebhookController } = await Promise.resolve().then(() => __importStar(require('../controllers/VideoWebhookController')));
        app.post('/api/webhooks/video', VideoWebhookController.handleVideoWebhook);
    });
    (0, vitest_1.it)('updates Version status to COMPLETED and publishes SHOT_UPDATED for video readiness', async () => {
        const payload = {
        ...
        (0, vitest_1.expect)(VersionRepository.updateVersionStatus).toHaveBeenCalledWith(payload.versionId, 'COMPLETED', vitest_1.expect.objectContaining({
