...
jest.mock('../providers/ModelProvider', () => ({
    ModelProvider: {
...
describe('OrchestrationService.startGenerationFlow', () => {
    it('persists a PENDING version and calls ModelProvider with webhook URL', async () => {
        ...
        expect(VersionRepository.insertVersion).toHaveBeenCalledWith(expect.objectContaining({
            shotId: validShot.shotId,
            status: 'PENDING'
        }));
        expect(ModelProvider.requestGeneration).toHaveBeenCalledTimes(1);
        expect(ModelProvider.requestGeneration).toHaveBeenCalledWith(expect.objectContaining({
            shot: validShot,
            webhookUrl
        }));
    });
